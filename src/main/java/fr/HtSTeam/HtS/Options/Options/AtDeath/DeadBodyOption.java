...
public class DeadBodyOption extends Option<Boolean> implements DeathTrigger, EndTrigger {
	...
	// Nms.packetPlayOutPlayerInfo_PlayerInfoDataClass;
	...
	public static boolean spawnCorpse(fr.HtSTeam.HtS.Player.Player p) {
		try {
			Object pos = Nms.blockPositionClass.getConstructor(int.class, int.class, int.class)
					.newInstance(p.getLocation().getBlockX(), 0, p.getLocation().getBlockZ());
			// BlockPosition pos = new BlockPosition(p.getLocation().getBlockX(), 0,
	...
	@Deprecated
	public static boolean spawnCorpse(Player p, Object pos)
		...
		temp.put(p.getLocation(), entityID.intValue()); // entityID.intValue() clones the instance so as to avoid
														// synchronization issues.

		corpses.put(HtSPlayer, temp.entrySet().iterator().next());

		// Constructor<?> ChatMessage =
		// Nms.iChatBaseComponentClass.getConstructor(String.class, Object[].class);
		...
		// Field infoAction_ADD_PLAYER =
		// Nms.packetPlayOutPlayerInfo_PlayerInfoActionEnum.getDeclaredField("ADD_PLAYER");
		...
		Object gameProfile = Nms.gameProfileClass.getConstructors()[0].newInstance(getUniqueId.invoke(cPlayer1),
				getName.invoke(cPlayer1));
		// GameProfile prof = new GameProfile(p1.getUniqueId(), p1.getName());

		// Object packetInfo =
		// PacketInfo.getConstructors()[1].newInstance(infoAction_ADD_PLAYER);
		// PacketPlayOutPlayerInfo packetInfo = new
		// PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.ADD_PLAYER);
		// Object data = PlayerData.getConstructors()[0].newInstance(gameProfile, 0,
		// gm_SURVIVAL, ChatMessage.newInstance("", new Object[0]));
		// PacketPlayOutPlayerInfo.PlayerInfoData data = packetInfo.new
		// PlayerInfoData(prof, 0, EnumGamemode.SURVIVAL, new ChatMessage("", new
		// Object[0]));
		...
		// PacketPlayOutNamedEntitySpawn packetEntitySpawn = new
		...
		// PacketPlayOutEntityTeleport packetTeleport = new
		...
		// PacketPlayOutEntityTeleport packetTeleportDown = new
		// PacketPlayOutEntityTeleport();
		...
		for (Player player : Bukkit.getOnlinePlayers()) {
			...
			if (player != p) {
				...
				// Method sendPacket = Nms.playerConnectionClass.getDeclaredMethod("sendPacker",
				// Nms.packetClass);
				// Field playerConnection =
				// Nms.entityPlayerClass.getDeclaredField("playerConnection");

				// sendPacket.invoke(playerConnection.get(getHandle.invoke(pl)), packetInfo);
				// sendPacket.invoke(playerConnection.get(getHandle.invoke(pl)),
				// packetEntitySpawn);
				// sendPacket.invoke(playerConnection.get(getHandle.invoke(pl)),
				// packetTeleportDown);
	...
	@Deprecated
	public static boolean destroyCorpse(Player p)
		...
		// Method getHandle = CPlayer.getDeclaredMethod("getHandle");
		// Method sendPacket = Nms.playerConnectionClass.getDeclaredMethod("sendPacket",
		// Nms.packetClass);
		// Field playerConnection =
		// Nms.entityPlayerClass.getDeclaredField("playerConnection");

		// Object packet =
		// Nms.packetPlayOutEntityDestroyClass.getConstructor(int[].class).newInstance(corpses.get(player).getValue());
		// PacketPlayOutEntityDestroy packet = new
		// PacketPlayOutEntityDestroy(corpses.get(p).getValue());
	...
	private static Object clonePlayerDatawatcher(Player player, int currentEntityID)
		...
		Object eh = EHuman.getConstructors()[0].newInstance(getHandle.invoke(CWorld.cast(player.getWorld())),
				getProfile.invoke(CPlayer.cast(player)));
		/**
		 * EntityHuman eh = new EntityHuman(((CraftWorld)
		 * player.getWorld()).getHandle(), ((CraftPlayer) player).getProfile()) { public
		 * boolean u() {return false;} public boolean isSpectator() {return false;} };
		 */
	...
	private static <T> void setValue(Object instance, String fieldName, T value)
			throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
